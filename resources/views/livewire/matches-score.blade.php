...
new class extends Component {
     ...
     */
    public array $setDurationLabels = [];
    ...
    private function syncGameState(): void
    {
        ...
        if ($result['is_complete']) {
            ...
            $this->setDurationLabels = $completedSets->values()->map(fn($set, int $index): string => sprintf('Set %d (%d-%d) %s', $index + 1, (int) $set->player_one_score, (int) $set->player_two_score, $this->formatDuration($set->duration_seconds)))->all();
        } else {
            $this->matchWinnerName = '—';
            $this->matchFinalResult = '—';
            $this->matchDurationLabel = '—';
            $this->setDurationLabels = [];
        }
    ...
    @if ($showMatchDoneOverlay)
        <div class="absolute inset-0 z-55 flex items-center justify-center bg-background/90 backdrop-blur-sm"
            ...
            <div class="relative w-full max-w-2xl rounded-3xl border border-border bg-card p-6 shadow-xl sm:p-8">
                ...
                <p class="mt-1 text-center text-6xl font-display font-bold text-foreground sm:text-7xl">
                    {{ $matchFinalResult }}
                </p>

                <div class="mt-6 rounded-2xl border border-border/70 bg-background/70 p-4">
                    <p class="text-center text-sm font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                        Match Duration
                    </p>
                    <p class="mt-1 text-center text-2xl font-display font-semibold text-foreground">
                        {{ $matchDurationLabel }}
                    </p>

                    <p class="mt-4 text-center text-sm font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                        Set Durations
                    </p>
                    <ul class="mt-2 space-y-1.5">
                        @forelse ($setDurationLabels as $index => $setDurationLabel)
                            <li wire:key="match-done-set-duration-{{ $index }}"
                                class="rounded-lg border border-border/60 bg-card px-3 py-2 text-center text-sm font-semibold text-foreground">
                                {{ $setDurationLabel }}
                            </li>
                        @empty
