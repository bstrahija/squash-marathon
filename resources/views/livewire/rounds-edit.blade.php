...
new class extends Component {
    ...
    public function mount(int $roundId): void
    {
        ...
        $this->groupOnePlayerIds = $groupOne
            ? $groupOne->users->pluck('id')->map(fn (int $id): int => (int) $id)->values()->all()
            : [];

        $this->groupTwoPlayerIds = $groupTwo
            ? $groupTwo->users->pluck('id')->map(fn (int $id): int => (int) $id)->values()->all()
            : [];
    }
